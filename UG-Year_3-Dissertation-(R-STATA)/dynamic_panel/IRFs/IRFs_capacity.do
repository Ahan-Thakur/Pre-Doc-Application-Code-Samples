...
forvalues h = 0/`H' {
    quietly reghdfe ln_kpc_lead`h' Capacity ///
        L.ln_capital_per_capita, ///
        absorb(ncountrycode year)

    matrix IRF[`=`h'+1', 1] = `h'
    matrix IRF[`=`h'+1', 2] = _b[Capacity]
    matrix IRF[`=`h'+1', 3] = _se[Capacity]
}
...
forvalues h = 0/`H' {
    quietly reghdfe ln_hc_lead`h' Capacity ///
        L.ln_human_capital, ///
        absorb(ncountrycode year)
...
twoway (rarea ub lb horizon, color(gs15%80)) ///
       (line irf horizon, lwidth(medthick) lcolor(navy)), ///
       title("Impulse Response of log human capital to Capacity Shock") ///
       yline(0, lpattern(dash)) ///
       xtitle("Years after Shock") ytitle("Response") ///
       legend(on)
...
forvalues h = 0/`H' {
    quietly reghdfe ln_tfp_lead`h' Capacity ///
        L.ln_rtfpna, ///
        absorb(ncountrycode year)
...
twoway (rarea ub lb horizon, color(gs15%80)) ///
       (line irf horizon, lwidth(medthick) lcolor(navy)), ///
       title("Impulse Response of log TFP(na) to Capacity Shock") ///
       yline(0, lpattern(dash)) ///
       xtitle("Years after Shock") ytitle("Response") ///
       legend(on)
